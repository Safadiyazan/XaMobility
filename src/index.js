...
async function LoadSimulation() { //data
  // try {
    ...
    function calculateOrientation(positionProperty) {
      ...
      const customOrientationProperty = new CallbackProperty((time, result) => {
        try {
          ...
          const maxRoll = CesiumMath.toRadians(15);  // Max roll in radians
          const maxYaw = CesiumMath.toRadians(180);  // Max yaw in radians
          ...
          const yaw = CesiumMath.clamp(pitchRollYaw.yaw, -maxYaw, maxYaw);//  pitchRollYaw.yaw; // 
          ...
          return VelocityOrientationProperty(positionProperty);
        }
      }, false);
