function [TFC,EC] = CalTFC_Ri(TFC,EC,SimInfo,ObjAircraft,Settings)
dtS = SimInfo.dtS;
dtM = SimInfo.dtM;
t = SimInfo.t;
...
for i=1:size(uniRi,1)
    ...
    TFC.Ri(i).nexit(t/dtM) = sum(NexitAircraft);
    ...
    EC.Ri(i).ECt(t/dtM) = sum(EC.ECdt(tk0:tk1,:).*Current_RiActiveAircraftdt,'all');
    EC.Ri(i).ECdq(t/dtM) = sum(EC.ECdt(tk0:tk1,:).*Current_RiDepQueueAircraftdt,'all');
    EC.Ri(i).ECbq(t/dtM) = sum(EC.ECdt(tk0:tk1,:).*Current_RiBouQueueAircraftdt,'all');
    EC.Ri(i).EC(t/dtM) = EC.Ri(i).ECt(t/dtM)+EC.Ri(i).ECdq(t/dtM)+EC.Ri(i).ECbq(t/dtM);
    EC.Ri(i).ECt_TTD(t/dtM) = EC.Ri(i).ECt(t/dtM)/TFC.Ri(i).TTD(t/dtM);
    EC.Ri(i).ECt_TTT(t/dtM) = EC.Ri(i).ECt(t/dtM)/TFC.Ri(i).TTT(t/dtM);
    EC.Ri(i).ECt_N(t/dtM) = EC.Ri(i).ECt(t/dtM)./TFC.Ri(i).ni(t/dtM);
    EC.Ri(i).ECt_G(t/dtM) = EC.Ri(i).ECt(t/dtM)./TFC.Ri(i).G(t/dtM);
end
end
