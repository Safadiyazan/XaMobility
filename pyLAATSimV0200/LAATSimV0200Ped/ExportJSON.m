function [scenarioName] = ExportJSON(SceStr,SimInfo,PedAircraft,TFC,Settings)
...
for i = 1:M
    ...
    ObjPedData{i} = struct(...
    'AMI', PedAircraft(i).AMI,...
    'stat', stat,...
    'tda',max(PedAircraft(i).tdp,0),... % aircraft departure time [s]
    'taa',min(PedAircraft(i).taa,SimInfo.tf),... % aircraft arrival time [s]
    'rs',PedAircraft(i).rs,... % aircraft safety radius [m]
    'rd',PedAircraft(i).rd,... % aircraft detection radius [m]
    'x', x,...
    'y', y,...
    'z', z...
    );
