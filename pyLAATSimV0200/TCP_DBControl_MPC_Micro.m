...
if (t~=0)&&(mod(t,dtC)==0)&&(~isempty(TFC.CS))&&(Settings.TFC.TCP_DBController_MPC)
    [ud_opt,ub_opt,MPCSim] = TCP_DBControl_MPC_Macro(SimInfo,ObjAircraft,Settings,TFC,t);
    ...
    % --- DEBUG DISPLAY ---
    k = t/dtC;
    disp(['[INFO] t=', num2str(t), 's | TCP_DBControl_MPC_Micro (Outputs):']);
    disp(['  > Optimal Departure Control (ud_opt):    [', num2str(ud_opt), ']']);
    disp(['  > Optimal Boundary Control (ub_opt):     [', num2str(ub_opt), ']']);
    disp('  > Boundary Queue In (NbqijIndt):');
    disp(MPCSim.NbqijIndt);
    disp('  > Boundary Queue Out (NbqijOutdt):');
    disp(MPCSim.NbqijOutdt);
end
%% Set Queue Boundary state
if (t~=0)&&(~isempty(TFC.CS))&&(Settings.TFC.TCP_DBController_MPC)
    ...
    %     disp('[DEBUG] TCP_DBControl_MPC_Micro: Check MPC solution, especially Nin/Nout values.');
    %     disp('[DEBUG] TCP_DBControl_MPC_Micro: Check simulation duration and values.');
end
%% Change DepartTime
...
%     disp('[WARN] TCP_DBControl_MPC_Micro: ADD IN OUT CONCEPT!');
