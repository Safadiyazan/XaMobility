function [ud_opt,ub_opt,MPCSim] = MPC_CoupledModel_2L2R_V2_LeM(SimInfo,ObjAircraft,Settings,TFC,ttt)
%%
warning('TODO: only worked for DC-POE LeM, should be updated as 3L')
import casadi.*
...
% x12Q_max = nc1; x12Q_min = 0;
% x21Q_max = nc2; x21Q_min = 0;
...
xCon = [x11T_max,x12T_max,x21T_max,x22T_max,x1T_max,x2T_max,xT_max]'; % x1DQ_max,x2DQ_max,x12Q_max,x21Q_max
%% Define States
x11T=SX.sym('x11T');
x12T=SX.sym('x12T');
x21T=SX.sym('x21T');
...
rhs = [
    %-----dn11T----
    d11*ud1 + bij(x21T,x2T)*(G2(x2T))*ub21  - bij(x11T,x1T)*(G1(x1T));
    %-----dn12T----
    d12*ud1 - bij(x12T,x1T)*(G1(x1T))*ub12;
    %-----dn21T----
    d21*ud2 - bij(x21T,x2T)*(G2(x2T))*ub21;
    %-----dn22T----
    d22*ud2 + bij(x12T,x1T)*(G1(x1T))*ub12 - bij(x22T,x2T)*(G2(x2T));
    %-----dn1T----
    d11*ud1 + bij(x21T,x2T)*(G2(x2T))*ub21 - bij(x11T,x1T)*(G1(x1T)) + d12*ud1 - bij(x12T,x1T)*(G1(x1T))*ub12;
    %-----dn2T----
    d21*ud2 - bij(x21T,x2T)*(G2(x2T))*ub21 + d22*ud2 + bij(x12T,x1T)*(G1(x1T))*ub12 - bij(x22T,x2T)*(G2(x2T));
    %-----dnT----
    d11*ud1 + bij(x21T,x2T)*(G2(x2T))*ub21 - bij(x11T,x1T)*(G1(x1T)) + d12*ud1 - bij(x12T,x1T)*(G1(x1T))*ub12 + d21*ud2 - bij(x21T,x2T)*(G2(x2T))*ub21 + d22*ud2 + bij(x12T,x1T)*(G1(x1T))*ub12 - bij(x22T,x2T)*(G2(x2T));
    % %-----dn1DQ----
    % (d11+d12)*(1-ud1);
    % %-----dn2DQ----
    % (d22+d21)*(1-ud2);
...
jjj=jjj+1;
args.lbx(jjj:n_states:n_states*(Np+1),1) = xT_min; %state xiq1 lower bound
args.ubx(jjj:n_states:n_states*(Np+1),1) = xT_max; %state xiq1 upper bound
% jjj=jjj+1;
