...
% Assuming DBC is applied to the first two regions of the first flying layer.
% The indices might need to be more dynamic if controlling other layers.
flying_region1_idx = 3;
flying_region2_idx = 4;

% Departure regions are typically layer 0
dep_region1_idx = 1; % This might need to be found dynamically if structure changes
dep_region2_idx = 2;

% Boundary regions are typically layer 2
boundary_region1_idx = 5;
boundary_region2_idx = 6;
if size(Settings.Airspace.Layers,2) ~= 3
    error('[INFO] Boundary Control Policy should be setup with 3 layers, when the third one is for boundary queues. Please change the number of layer into 3 -> Ri.zn = 3;')
end

nc1 = Settings.TFC.Ri(flying_region1_idx).nci;
nc2 = Settings.TFC.Ri(flying_region2_idx).nci;
n1max = 3*nc1;
n2max = 3*nc2;

nijt = [sum(reshape(TFC.Ri(flying_region1_idx).nid(ttt/dT,:),2,[]),2)',sum(reshape(TFC.Ri(flying_region2_idx).nid(ttt/dT,:),2,[]),2)'];
nit = [TFC.Ri(flying_region1_idx).ni(ttt/dT), TFC.Ri(flying_region2_idx).ni(ttt/dT)];
ndqit = [TFC.Ri(dep_region1_idx).ndqi(ttt/dT), TFC.Ri(dep_region2_idx).ndqi(ttt/dT)];

% Boundary queues are between flying layers, assuming they are stored at specific indices
nbqiht = [TFC.Ri(boundary_region1_idx).nbqi(ttt/dT), TFC.Ri(boundary_region2_idx).nbqi(ttt/dT)];

qijt = [TFC.Ri(dep_region1_idx).qinij(ttt/dT,1), TFC.Ri(dep_region1_idx).qinij(ttt/dT,2),TFC.Ri(dep_region2_idx).qinij(ttt/dT,1), TFC.Ri(dep_region2_idx).qinij(ttt/dT,2)]./dT;
d11 = qijt(1);
d12 = qijt(2);
d21 = qijt(3);
d22 = qijt(4);
...
nb_controls = 4;
%% Functions
G1 = Settings.TFC.Ri(flying_region1_idx).funGn;
G2 = Settings.TFC.Ri(flying_region2_idx).funGn;
...
% --- DEBUG DISPLAY ---
disp(['[INFO] t=', num2str(ttt), 's | TCP_DBControl_MPC_Macro (Inputs):']);
disp(['  > Initial State (x0): [nijt, nit, ndqit, nbqiht]']);
disp(['    ', num2str(x0')]);
